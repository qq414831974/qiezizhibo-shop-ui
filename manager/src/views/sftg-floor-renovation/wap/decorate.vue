<template>
  <div class="decorate">
    ...
    <sftgDialog ref="sftgDialog" @selectedLink="selectedLink" @selectedGoodsData="selectedGoodsData"></sftgDialog>
...
import ways from "@/views/sftg-dialog/wap.js"; // 选择链接的类型
export default {
  components: {
    ossManage,
  },
  ...
  methods: {
    ...
    // 打开图片选择器
    sftgDialogFlag(flag) {
      this.$refs.sftgDialog.goodsFlag = flag;
      this.$refs.sftgDialog.flag = true;
    },

    // 点击链接赋值一个唯一值，并将当前选择的模块赋值
    clickLink(val, index) {
      this.selectedLinks = val;
      this.sftgDialogFlag(false);
    },
    //点击图片解析成base64
